...
MessageType = Literal[
    "attendance", "incident", "question", "task", "substitution", "unknown"
]
...
{
  "type": "attendance" | "incident" | "question" | "task" | "substitution" | "unknown",
...
- "substitution" — запрос на поиск замены отсутствующему учителю
  (например: "Иванов заболел, кто заменит 3-й урок?", "найди замену Петровой на 2 урок в четверг",
  "кто может подменить Сидорова сегодня на пятом?").
  В details извлекай: absent_fio (ФИО отсутствующего), lesson_number (число 1-12),
  day_of_week (число 1-7: пн=1, вт=2, ..., вс=7; либо null, если не указано).
...
async def classify_message(text: str) -> Classification:
    ...
    if msg_type not in ("attendance", "incident", "question", "task", "substitution", "unknown"):
        msg_type = "unknown"
